...
class OmorfiWrapper(object):
    ...
    def __init__(self, transducer_file):
        ...
        self.poll=None
        #does not use subprocess, uses external omorfi server
        #self.restart()
    ...
    def lookup(self, word,model):
        #calls omorfiserver to get word instead of polling subprocess
        #print >> sys.stderr, "OmorfiWrapper, lookup: model=%s, word=%s" % (model,word)
        data = {}
        data['model']=model
        data['word']=word
        url_values = urllib.urlencode(data)
        url = 'http://127.0.0.1:9876/omorfi'
        full_url = url + '?' + url_values
        #print >> sys.stderr, "OmorfiWrapper, url: %s" % full_url
        response = urllib2.urlopen(full_url)
        data = response.read()
        #print >> sys.stderr, "OmorfiWrapper, data: %s" % data
        ...
        #print >> sys.stderr, "Got %d results, returning" % len(results)
        return results


    def lookup_old(self, word):
